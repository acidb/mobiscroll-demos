...
calendar = mobiscroll.eventcalendar('#demo-restaurant-shift-management', {
    view: {
        timeline: {
            type: 'week',
            eventList: true,
...
document.querySelectorAll('.md-shift-filter').forEach(function (elm) {
    elm.addEventListener('change', function () {
        var selected = [];
        var displayedSlots = [];

        document.querySelectorAll('.md-shift-filter:checked').forEach(function (el) {
            selected.push(+el.value);
            mobiscroll.getInst(el).setOptions({ disabled: false });
        });
        ...
        if (selected.length === 1) {
            var lastElm = document.querySelectorAll('.md-shift-filter[value="' + selected[0] + '"]')[0];
            mobiscroll.getInst(lastElm).setOptions({ disabled: true });
        }
