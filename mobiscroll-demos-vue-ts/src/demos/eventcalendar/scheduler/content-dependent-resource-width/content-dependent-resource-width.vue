...
const getIcons = (resource: MbscResource) => {
  let icons = ''

  if (resource.inOffice) icons += inOfficeSvg
  if (resource.role === 'developer') icons += developerSvg
  if (resource.role === 'designer') icons += designerSvg
  if (resource.role === 'support') icons += supportSvg
  if (resource.vacation) icons += vacationSvg
  if (!resource.available) icons += availableSvg
...
<template>
  <!-- dragOptions -->
  <MbscEventcalendar :data="myEvents" groupBy="date" :resources="myResources" :view="myView">
    <template #resource="resource">
      <div>
        <div class="mds-var-col-resource-name">{{ resource.name }}</div>
        <div v-html="getIcons(resource)"></div>
      </div>
    </template>
  </MbscEventcalendar>
</template>
