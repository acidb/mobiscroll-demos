...
const timelineInst = ref<any>(null)
const myInvalids = ref<MyBlockedTimes[]>(blockedOutTimes)
...
const myDefaultEvent = (args: MbscNewEventData) => {
  if (timelineInst.value!.instance) {
    const events = timelineInst.value.instance
      .getEvents(args.start, new Date(+args.start + 3600000))
      .filter((e: MyEvent) => e.resource === args.resource)
...
const handelEventCreate = (args: MbscEventCreatedEvent) => {
  ...
  if (event.shift) {
    // Tasks was created
    const shift: MyEvent = { ...overlapEvents[0] }
    const shiftIndex = myEvents.value.findIndex((x) => x.id === event.shift)
    const newEventList: MyEvent[] = [...myEvents.value]

    if (overlapEvents.length > 2) {
      // Prevent overlap
      myEvents.value = newEventList
      toastMessage.value = 'No space for task'
...
const handelEventDelete = (args: MbscEventDeletedEvent) => {
  let newEventList = [...myEvents.value]
  const event = args.event
  const evIndex = myEvents.value.findIndex((x) => x.id === event.id)
...
const handelEventDragStart = (args: MbscEventDragEvent) => {
  const inst = args.inst!
  ...
  if (event.tasks) {
    ...
    myInvalids.value = [...blockedOutTimes, ...tempInvalid]
  } else {
    // Subtask
...
const handelEventDragEnd = () => {
  myInvalids.value = blockedOutTimes
}

const handelEventUpdate = (args: MbscEventUpdateEvent) => {
  const inst = args.inst!
    ...
    if (isResize || startDiff === endDiff) {
      ...
      tasks.forEach((task: MyEvent, i) => {
        ...
        if (isResize) {
          ...
          task.end = new Date(
            Math.min(+task.start + (+taskEnd - +taskStart), +new Date(event.end as Date))
          )
        } else {
...
const handelEventUpdateFailed = (args: MbscEventUpdateFailedEvent) => {
...
<template>
  <MbscEventcalendar
    ...
    @event-created="handelEventCreate"
    @event-deleted="handelEventDelete"
    @event-updated="handelEventUpdate"
    @event-drag-start="handelEventDragStart"
  ...
  >
    <template #timelineEventContent="args">
      ...
      <span className="mds-task-hours">
        -
        {{
          (+args.endDate - +args.startDate) / 3600000 +
          ((+args.endDate - +args.startDate) / 3600000 === 1 ? 'h' : 'hrs')
        }}</span
      >
    </template>
  </MbscEventcalendar>
