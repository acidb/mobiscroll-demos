...
export default {
  init() {
    ...
    $(function () {
      ...
      var $calendar = $('#demo-sync-events-google-calendar');
      var $popup = $('#demo-sync-events-google-calendar-popup');
      ...
      function toggleContainers(loggedIn) {
        var $loggedOutCont = $('#google-cal-sign-in');
        var $loggedInCont = $('#google-cal-get-calendars');
      ...
      function onSignedIn() {
        ...
        googleCalendarSync
          ...
          .then(function (calendars) {
            ...
            var $calendarList = $('#google-cal-list');
      ...
      var popup = $popup
        .mobiscroll()
        .popup({
          anchor: $('#google-cal-get-calendars')[0],
          width: 400,
          touchUi: false,
          showOverlay: false,
          scrollLock: false,
      ...
      // sign in
      $calendar.on('click', '#google-cal-sign-in', function () {
        if (!googleCalendarSync.isSignedIn()) {
          googleCalendarSync.signIn().catch(onError);
        }
      });

      $calendar.on('click', '#google-cal-today', function () {
        inst.navigate(new Date());
      });

      $calendar.on('click', '#google-cal-get-calendars', function () {
        popup.open();
      });

      // switch click
      $popup.on('change', '.google-calendar-switch', function (ev) {
      ...
      $popup.on('click', '#google-cal-sign-out', function () {
