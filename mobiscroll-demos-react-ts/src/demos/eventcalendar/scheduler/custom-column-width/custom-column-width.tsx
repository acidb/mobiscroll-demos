...
import './custom-column-width.css';
...
const App: FC = () => {
  const [myEvents, setEvents] = useState<MbscCalendarEvent[]>([
    ...
    { start: 'dyndatetime(y,m,d+9,10)', end: 'dyndatetime(y,m,d+9,1</SegmentedGroup>2)', title: 'Stand-up' },
  ...
  const [myResEvents, setResEvents] = useState<MbscCalendarEvent[]>([
  ...
  const [myResources2, setResources2] = useState<MbscResource[]>([]);
  const [showResources, setShowResources] = useState<string>('show');

  const [myClass, setClass] = useState<string>('my-col-width-small');
  ...
  const toggleResources = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setResources(value === 'show' ? myResources2 : []);
      setEvents(value === 'show' ? myResEvents : []);
    },
    [myResources2, myResEvents],
  );

  return (
    <Page>
      <SegmentedGroup value={myClass} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setClass(e.target.value)}>
        <Segmented value="my-col-width-small">50px</Segmented>
        <Segmented value="my-col-width-medium">100px</Segmented>
        <Segmented value="my-col-width-large">150px</Segmented>
      </SegmentedGroup>
      <SegmentedGroup value={showResources} onChange={toggleResources}>
        <Segmented value="show">With resources</Segmented>
        <Segmented value="hide">Without resources</Segmented>
      </SegmentedGroup>
      <Eventcalendar
        // drag
        cssClass={myClass}
