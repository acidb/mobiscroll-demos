...
const App: FC = () => {
  ...
  const renderCustomResource = useCallback((resource: MbscResource) => {
    ...
    if (resource.inOffice) icons.push(inOfficeSvg);
    if (resource.role === 'developer') icons.push(developerSvg);
    if (resource.role === 'designer') icons.push(designerSvg);
    if (resource.role === 'support') icons.push(supportSvg);
    if (resource.vacation) icons.push(vacationSvg);
    if (!resource.available) icons.push(availableSvg);

    return (
      <div>
        <div className="mds-var-col-resource-name">{resource.name}</div>
        <div dangerouslySetInnerHTML={{ __html: icons.join('') }} />
      </div>
    );
  }, []);
