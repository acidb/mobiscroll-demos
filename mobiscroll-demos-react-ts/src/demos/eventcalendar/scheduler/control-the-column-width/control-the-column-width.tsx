...
const App: FC = () => {
  ...
  const [myResEvents] = useState<MbscCalendarEvent[]>([
  ...
  const [myResources] = useState<MbscResource[]>([
  ...
  return (
    <Page>
      ...
      <SegmentedGroup value={showResources} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setShowResources(e.target.value)}>
      ...
      <Eventcalendar
        // drag
        cssClass={myClass}
        data={showResources === 'show' ? myResEvents : myEvents}
        resources={showResources === 'show' ? myResources : []}
        groupBy="date"
        view={myView}
      />
    </Page>
