...
function App() {
  ...
  const todayRef = useRef<Date>(new Date(nowRef.current.getFullYear(), nowRef.current.getMonth(), nowRef.current.getDate()));
  const calendarRef = useRef<Eventcalendar | null>(null);
  const myEventsRef = useRef<DispatchEvent[]>(initEvents());
  const externalEventsRef = useRef<ExternalJob[]>(initExternalEvents());
  const scheduledJobIdsRef = useRef<number[]>([]);
  const myInvalidsRef = useRef<MbscCalendarEvent[]>([]);
  const currentViewStartRef = useRef<Date>(todayRef.current);
  const currentViewEndRef = useRef<Date>(new Date(todayRef.current.getTime() + 7 * 24 * 60 * 60 * 1000));
  const currentRangeStartRef = useRef<Date>(todayRef.current);
  const currentRangeDaysRef = useRef<number>(7);
  const pendingRangeStartRef = useRef<Date>(todayRef.current);
  ...
  const [myEvents, setMyEvents] = useState<DispatchEvent[]>(() => myEventsRef.current);
  const [filteredResources, setFilteredResources] = useState<CapacityGroup[]>([]);
  const [maintenanceInvalids, setMaintenanceInvalids] = useState<MbscCalendarEvent[]>([]);
  const [calendarInvalid, setCalendarInvalid] = useState<MbscCalendarEvent[]>([]);
  const [zoomLevel, setZoomLevel] = useState<number>(3);
  const [calView, setCalView] = useState<MbscEventCalendarView>(() => buildViewConfig(7));
  const [refDate, setRefDate] = useState<Date>(() => todayRef.current);
  const [scheduledJobIds, setScheduledJobIds] = useState<number[]>([]);
  const [visibleJobs, setVisibleJobs] = useState<ExternalJob[]>([]);
  const [jobListItems, setJobListItems] = useState<JobListItem[]>([]);
  const [filters, setFilters] = useState<Filters>(() => {
    const f: Filters = { operational: true, maintenance: true };
    allResources.forEach((cat) => { f[cat.id as string] = true; });
    return f;
  });
  const [tempFilters, setTempFilters] = useState<Filters>({});
  const [statusFilters, setStatusFilters] = useState<Record<string, boolean>>({ scheduled: true, 'in progress': true, completed: true });
  ...
  const computeFilteredResources = useCallback((currentFilters?: Filters, currentSearchQuery?: string): CapacityGroup[] => {
  ...
  const getStatusFilteredEvents = useCallback((events: DispatchEvent[], currentStatusFilters?: Record<string, boolean>): DispatchEvent[] => {
    const sf = currentStatusFilters || statusFilters;
    return events.filter((ev) => sf[getEffectiveStatus(ev)] !== false);
  ...
  const buildMaintenanceInvalids = useCallback((): MbscCalendarEvent[] => {
  ...
  const refreshJobList = useCallback((currentFilters?: Filters, currentScheduledIds?: number[], rangeStart?: Date, rangeDays?: number) => {
  ...
  const addActualEvents = useCallback((events: DispatchEvent[]) => {
    const now = nowRef.current;
  ...
  const findFirstSlot = useCallback((draggedEvent: DispatchEvent): { start: Date; end: Date } | null => {
    const now = new Date();
  ...
  const applyDaysRange = useCallback((days: number, startDate?: Date) => {
    ...
    setTimeout(() => (calendarRef.current as any)?.navigate(navigateTo), 100);
  ...
  const invalidateResources = useCallback((event: DispatchEvent): number | null => {
    const now = new Date();
    nowRef.current = now;
    ...
    for (const group of allResources) {
      for (const truck of group.children) {
        ...
        if (isValid) {
          const truckEvents = (calendarRef.current as any)?.getEvents()?.filter((ev: DispatchEvent) => ev.resource === truck.id) || [];
  ...
  const onEventCreated = useCallback((args: MbscEventCreatedEvent) => {
    ...
    if (args.action === 'externalDrop') {
      ...
      (calendarRef.current as any)?.updateEvent(scheduledEvent);
      myEventsRef.current = [...myEventsRef.current, scheduledEvent];
      setMyEvents([...myEventsRef.current]);
      setVisibleJobs((prev) => prev.filter((j) => j.id !== event.id));
      setJobListItems((prev) => prev.filter((item) => item.type !== 'job' || item.job!.id !== event.id));
    }
    showToast(event.from + ' → ' + event.to + ' added');
  }, [showToast]);

  const onEventCreateFailed = useCallback((args: MbscEventCreateFailedEvent) => {
    ...
    if (slot) {
      ...
      (calendarRef.current as any)?.addEvent(draggedEvent);
      if (args.action === 'externalDrop') {
        scheduledJobIdsRef.current = [...scheduledJobIdsRef.current, draggedEvent.id as number];
        setScheduledJobIds([...scheduledJobIdsRef.current]);
        setVisibleJobs((prev) => prev.filter((j) => j.id !== draggedEvent.id));
        setJobListItems((prev) => prev.filter((item) => item.type !== 'job' || item.job!.id !== draggedEvent.id));
      }
      setTimeout(() => (calendarRef.current as any)?.navigateToEvent({ start: draggedEvent.start, resource: draggedEvent.resource }));
  ...
  const onEventUpdateFailed = useCallback((args: MbscEventUpdateFailedEvent) => {
    const draggedEvent = args.event as DispatchEvent;
    const slot = findFirstSlot(draggedEvent);
    if (slot) {
      draggedEvent.start = slot.start; draggedEvent.end = slot.end;
      (calendarRef.current as any)?.updateEvent(draggedEvent);
      setTimeout(() => (calendarRef.current as any)?.navigateToEvent({ start: draggedEvent.start, resource: draggedEvent.resource }));
      ...
      setTimeout(() => (calendarRef.current as any)?.navigateToEvent({ start: originalEvent.start, resource: originalEvent.resource }));
      showToast('No available slot found for ' + draggedEvent.from + ' → ' + draggedEvent.to);
    }
  }, [findFirstSlot, showToast]);

  const onEventDragStart = useCallback((args: MbscEventDragEvent) => {
    const event = args.event as DispatchEvent;
    const resourceToNavigate = invalidateResources(event);
    if (!event.resource) {
      if (resourceToNavigate && findFirstSlot(event)) {
        (calendarRef.current as any)?.navigateToEvent({ start: event.start, resource: resourceToNavigate });
      } else {
  ...
  const onPageLoaded = useCallback((args: MbscPageLoadedEvent) => {
    currentViewStartRef.current = args.firstDay as Date;
    currentViewEndRef.current = args.lastDay as Date;
  }, []);

  const renderResource = useCallback((resource: TruckResource) => {
      ...
      <div className="mbsc-flex-1-1">
        ...
        <button
          ...
          onClick={() => {
            ...
            allResources.forEach((cat) => { newFilters[cat.id as string] = true; });
    ...
    <div className="mbsc-flex mbsc-align-items-center mbsc-font mds-dispatch-search">
      ...
      <button
        ...
        onClick={(e) => {
          ...
          filterAnchorRef.current = e.target as HTMLElement;
  ...
  const renderScheduleEventContent = useCallback((data: { original: DispatchEvent }) => {
  ...
  const rangePopupResponsive = useMemo(() => ({
    xsmall: { display: 'bottom' as const, touchUi: true, buttons: [{ text: 'Apply', keyCode: 'enter', handler: applyPendingRange, cssClass: 'mbsc-popup-button-primary' }, 'cancel'] },
    custom: { breakpoint: 575, buttons: [] as MbscPopupButton[], display: 'anchored' as const, touchUi: false, scrollLock: false, maxWidth: 920 },
  }), [applyPendingRange]);

  const filterPopupButtons = useMemo<MbscPopupButton[]>(() => [
    'cancel',
    {
      ...
      <div className="mds-dispatch-custom-header mbsc-flex">
        ...
        <div className="mds-dispatch-header-right mbsc-flex">
          <div className="mds-dispatch-status-filter mbsc-flex">
            {([['scheduled', 'Scheduled'], ['in progress', 'In progress'], ['completed', 'Completed']] as [string, string][]).map(([val, label]) => (
          ...
          <button className="mbsc-button mbsc-button-outline mbsc-font mds-dispatch-now-btn" onClick={() => {
            ...
            const end = new Date(start.getTime() + (days - 1) * 24 * 60 * 60 * 1000);
            setRangeLabel(formatDate('MMM D', start) + ' – ' + formatDate('MMM D, YYYY', end));
            setTimeout(() => (calendarRef.current as any)?.navigate(now));
            refreshJobList();
          }}>Now</button>
        </div>
      </div>
      ...
      >
        <div className="mbsc-form-group">
          ...
          {allResources.map((res) => (
            <label key={res.id as string} style={{ display: 'flex', alignItems: 'center', padding: '4px 16px', cursor: 'pointer' }}>
              <input type="checkbox" checked={!!tempFilters[res.id as string]} onChange={(e) => setTempFilters((tf) => ({ ...tf, [res.id as string]: e.target.checked }))} style={{ marginRight: 8 }} />
              {res.name}
            </label>
          ))}
        </div>
                  ...
                  <Datepicker
                    ...
                    onChange={(args: MbscDatepickerChangeEvent) => {
                      if (args.value && calendarRangeValue?.[1]) {
                        pendingRangeStartRef.current = new Date(args.value as string);
