...
function App() {
  ...
  const fillPopup = useCallback((event, isEdit) => {
    setEdit(isEdit);
  ...
  const saveEvent = useCallback(
    (event) => {
      const start = shiftDates[0];
      const end = shiftDates[1] ? shiftDates[1] : shiftDates[0];
  ...
  const handleEventClick = useCallback(
    (args) => {
      ...
      setHeaderDate(formatDate('DDDD', new Date(event.start)) + ' ' + slot.name + ', ' + formatDate('D MMMM YYYY', new Date(event.start)));
      setPopupOpen(true);
    },
    [fillPopup],
  );
  ...
  const handleShiftNotesChange = useCallback((ev) => {
    setShiftNotes(ev.target.value);
  }, []);

  const handleShiftDatesChange = useCallback((args) => {
