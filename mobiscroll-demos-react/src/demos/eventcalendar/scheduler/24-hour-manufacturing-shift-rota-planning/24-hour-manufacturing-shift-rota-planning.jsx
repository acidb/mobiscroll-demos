...
function App() {
  ...
  const [redResources, setRedResources] = useState({});
  const [draggedEventStart, setDraggedEventStart] = useState(null);
  const [draggedEventEnd, setDraggedEventEnd] = useState(null);
  const [draggedEventResource, setDraggedEventResource] = useState(null);
  const [availableSlotOnHover, setAvailableSlotOnHover] = useState(null);
  ...
  const clearColorsForResource = useCallback((currentColors, resourceId, date) => {
  ...
  const handleEventCreate = useCallback(
    (args) => {
      ...
      const conflict = dayEvents.find((ev) => ev.resource === event.resource || +new Date(ev.start) === +new Date(event.start));
      if (conflict) {
        setToastMessage('Already assigned');
        setToastOpen(true);
        return false;
      }
      setColors((prev) => clearColorsForResource(prev, event.resource, event.start));
      const day = new Date(event.start);
      day.setHours(0, 0, 0, 0);
      setRedResources((prev) => ({ ...prev, [event.resource + day.toISOString()]: false }));
      setAvailableSlotOnHover(null);
  ...
  const handleEventDragStart = useCallback((args) => {
    setDraggedEventStart(args.event.start);
    setDraggedEventEnd(args.event.end);
    setDraggedEventResource(args.event.resource);
  }, []);
  ...
  const handleEventUpdate = useCallback(
    (args) => {
      ...
      const conflicts = dayEvents.filter(
        (ev) => ev.id !== event.id && (ev.resource === event.resource || +new Date(ev.start) === +new Date(event.start)),
      );
