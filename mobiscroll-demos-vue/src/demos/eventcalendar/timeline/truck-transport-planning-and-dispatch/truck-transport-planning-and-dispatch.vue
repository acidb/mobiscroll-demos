...
const statusFilters = ref(['scheduled', 'in progress', 'completed'])
...
const rangeStartInput = ref(null)
const rangeEndInput = ref(null)
...
function invalidateResources(event) {
  ...
  for (const group of allResources) {
    for (const truck of group.children) {
      ...
      if (isValid) {
        const truckEvents =
          calendarInst.value?.instance?.getEvents()?.filter((ev) => ev.resource === truck.id) || []
        const overlappingEvent = truckEvents.find((ev) => {
          const evEnd = ev.drop ? new Date(ev.drop[1]) : new Date(ev.end)
          return evEnd > windowStart && new Date(ev.start) < windowEnd
        })
...
function onEventCreated(args) {
  if (args.action === 'externalDrop') {
    ...
    jobListItems.value = jobListItems.value.filter(
      (item) => item.type !== 'job' || item.job.id !== args.event.id
    )
  }
  ...
  if (ev.status === 'actual') {
    ...
    if (ev.title === 'In progress') {
      msg =
        ...
        ' · running ' +
        formatDuration(new Date() - actualStart)
    } else {
                ...
                <div v-else ref="dragElements" class="mds-dispatch-jobs mbsc-flex">
                  ...
                  <div class="mds-dispatch-job-meta mbsc-flex-col mbsc-flex-none">
                    <span class="mds-dispatch-job-time"
                      >{{ formatDate('H:mm', new Date(item.job.pickup[0])) }} –
                      {{ formatDate('H:mm', new Date(item.job.drop[1])) }}</span
                    >
