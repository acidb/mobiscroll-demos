...
function getResourceMaintenanceStatus(resource, viewStart, viewEnd) {
...
const calendarInst = ref(null)
const myEventsData = ref(initEvents().map((e) => ({ ...e, start: e.pickup[0], end: e.drop[0], title: e.from + ' → ' + e.to })))
const externalEventsData = ref(allExternalEvents.map((e) => ({ ...e, start: e.start || e.pickup[0], end: e.end || e.drop[0], title: e.from + ' → ' + e.to })))
const scheduledJobIds = ref([])
const myInvalids = ref([])
const currentViewStart = ref(today)
const currentViewEnd = ref(new Date(today.getTime() + 7 * 24 * 60 * 60 * 1000))
const currentRangeStart = ref(today)
const currentRangeDays = ref(7)
const pendingRangeStart = ref(today)
const pendingRangeDays = ref(7)

const filteredResources = ref([])
const maintenanceInvalids = ref([])
const calendarInvalid = ref([])
const zoomLevel = ref(3)
const calView = ref(buildViewConfig(7))
const refDate = ref(today)
const visibleJobs = ref([])
const jobListItems = ref([])
const filters = ref({ operational: true, maintenance: true, cap1: true, cap2: true, cap3: true, cap4: true, cap5: true })
const tempFilters = ref({})
const statusFilters = ref({ scheduled: true, 'in progress': true, completed: true })
const searchQuery = ref('')
const rangeLabel = ref('')
const rangeSelectValue = ref('7')
const rangeInputsDisabled = ref(true)
const calendarRangeValue = ref(null)
...
function computeFilteredResources(currentFilters, currentSearchQuery) {
...
function getMaxAvailableCapacity(currentFilters) {
  const f = currentFilters || filters.value
  let max = 0
  allResources.forEach((group) => {
    if (f[group.id]) {
...
function refreshJobList(currentFilters, currentScheduledIds, rangeStart, rangeDays) {
  const now = nowRef.value
...
function invalidateResources(event) {
  ...
  for (const group of allResources) {
    for (const truck of group.children) {
      ...
      if (isValid) {
        const truckEvents = calendarInst.value?.instance?.getEvents()?.filter((ev) => ev.resource === truck.id) || []
        const overlappingEvent = truckEvents.find((ev) => { const evEnd = ev.drop ? new Date(ev.drop[1]) : new Date(ev.end); return evEnd > windowStart && new Date(ev.start) < windowEnd })
...
function onEventCreated(args) {
  if (args.action === 'externalDrop') {
    scheduledJobIds.value = [...scheduledJobIds.value, args.event.id]
    ...
    visibleJobs.value = visibleJobs.value.filter((j) => j.id !== args.event.id)
    jobListItems.value = jobListItems.value.filter((item) => item.type !== 'job' || item.job.id !== args.event.id)
  }
  showToast(args.event.from + ' → ' + args.event.to + ' added')
}

function onEventCreateFailed(args) {
  ...
  if (slot) {
    ...
    if (args.action === 'externalDrop') {
      scheduledJobIds.value = [...scheduledJobIds.value, draggedEvent.id]
...
function onEventUpdateFailed(args) {
  const draggedEvent = args.event
  const slot = findFirstSlot(draggedEvent)
  if (slot) {
    draggedEvent.start = slot.start; draggedEvent.end = slot.end
...
function onEventDragStart(args) {
  ...
  if (!args.event.resource) {
    if (resourceToNavigate && findFirstSlot(args.event)) {
      calendarInst.value?.instance?.navigateToEvent({ start: args.event.start, resource: resourceToNavigate })
    } else {
...
function onPageLoaded(args) {
  currentViewStart.value = args.firstDay
  currentViewEnd.value = args.lastDay
}

function getMaintenanceColor(resource) {
...
function getMaintenanceLabel(resource) {
  const status = getResourceMaintenanceStatus(resource, currentViewStart.value, currentViewEnd.value)
...
const filterPopupButtons = computed(() => [
  'cancel',
  {
...
function onSearchChange() {
  setTimeout(() => {
    filteredResources.value = computeFilteredResources(null, searchQuery.value)
  }, 300)
}

function onRangeSelectChange(args) {
...
function onRangeStartChange(args) {
  if (args.value && calendarRangeValue.value?.[1]) {
    pendingRangeStart.value = new Date(args.value)
...
function onRangeEndChange(args) {
...
function onCalendarRangeChange(args) {
...
<template>
    ...
    <div class="mds-dispatch-custom-header mbsc-flex">
      ...
      <div class="mds-dispatch-header-right mbsc-flex">
        <div class="mds-dispatch-status-filter mbsc-flex">
          <label v-for="[val, label] in [['scheduled', 'Scheduled'], ['in progress', 'In progress'], ['completed', 'Completed']]" :key="val" :class="'mds-dispatch-seg-' + val.replace(' ', '')" style="display: flex; align-items: center; cursor: pointer; padding: 0 8px">
            <input type="checkbox" style="margin-right: 4px" :checked="statusFilters[val]" @change="(e) => { statusFilters[val] = e.target.checked }" />
        ...
        <div class="mds-dispatch-zoom mbsc-flex">
          ...
          <input type="range" min="1" max="5" :value="zoomLevel" class="mds-dispatch-zoom-slider" @input="(e) => handleZoom(+e.target.value)" />
            ...
            <template v-for="item in jobListItems" :key="item.key">
              ...
              <div v-else ref="dragElements" class="mds-dispatch-jobs mbsc-flex">
                ...
                <div class="mds-dispatch-job-meta mbsc-flex-col mbsc-flex-none">
                  <span class="mds-dispatch-job-time">{{ formatDate('H:mm', new Date(item.job.pickup[0])) }} – {{ formatDate('H:mm', new Date(item.job.drop[1])) }}</span>
              ...
              <div class="mbsc-flex mbsc-align-items-center mbsc-font mds-dispatch-search">
                <label class="mbsc-flex-1-1">
                  <input
                    ...
                    placeholder="Search..."
                    :value="searchQuery"
                    @input="(e) => { searchQuery = e.target.value; onSearchChange() }"
  ...
  >
    <div class="mbsc-form-group">
      ...
      <div v-for="res in allResources" :key="res.id">
        <label style="display: flex; align-items: center; padding: 4px 16px; cursor: pointer">
          <input type="checkbox" :checked="!!tempFilters[res.id]" @change="(e) => { tempFilters[res.id] = e.target.checked }" style="margin-right: 8px" />
          {{ res.name }}
        </label>
      </div>
    </div>
    <div class="mbsc-form-group">
      ...
      <label style="display: flex; align-items: center; padding: 4px 16px; cursor: pointer">
        <input type="checkbox" :checked="!!tempFilters['maintenance']" @change="(e) => { tempFilters['maintenance'] = e.target.checked }" style="margin-right: 8px" />
        In maintenance/Maintenance planned
      </label>
      <label style="display: flex; align-items: center; padding: 4px 16px; cursor: pointer">
        <input type="checkbox" :checked="!!tempFilters['operational']" @change="(e) => { tempFilters['operational'] = e.target.checked }" style="margin-right: 8px" />
        Operational
      </label>
    </div>
  </MbscPopup>
