...
The starting point is being able to look at the same dataset through different lenses. The demo supports switching between an assignee view, where each row represents a person responsible for the work, and a work type view, where each row represents a task category. A segmented control in the header allows adjusting the time granularity of the timeline while keeping a full year in view, enabling users to switch between quarterly, monthly, or weekly breakdowns. The view selector, grouping toggle, and segmented control are all rendered via the [header](https://docs.mobiscroll.com/vue/eventcalendar/timeline#slot-header)
slot, keeping the controls integrated into the timeline header. The @if (pagemode == PageMode.Angular) { [resourceTemplate](https://docs.mobiscroll.com/vue/eventcalendar/timeline#template-resourceTemplate)  option } else if (pagemode == PageMode.Vue) { [resource](https://docs.mobiscroll.com/vue/eventcalendar/timeline#slot-resource)  slot } else { [renderResource](https://docs.mobiscroll.com/vue/eventcalendar/timeline#renderer-renderResource)  function } adapts the resource column accordingly, displaying employee avatars and job titles in assignee view, and work type color indicators in type view.

Once the right perspective is selected, individual events are not rendered one by one. Using the [timelineEvent](https://docs.mobiscroll.com/vue/eventcalendar/timeline#slot-timelineEvent)
slot, related tasks are automatically grouped into consolidated blocks per client and per quarter. These blocks can be expanded to reveal the individual tasks inside, or collapsed to keep the timeline readable at scale.
