...
To see this example live, check it out on our [demo page](https://demo.mobiscroll.com/vue/scheduler/doctors-availability-calendar-planner#).
...
To create the visual indicator, we added extra resource columns with a narrow width, styled using a custom CSS class set via the `cssClass` property in the resource data. The availability track is shown using events placed on these columns, with their content hidden via schedulerEvent option and resized responsively based on the narrow resource column width. Time ranges outside each doctor's working hours are disabled to reflect their actual schedule.
