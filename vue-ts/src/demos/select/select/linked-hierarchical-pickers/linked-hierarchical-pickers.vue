...
const divDisabled = ref<boolean>(true)
const subDisabled = ref<boolean>(true)
const divData = ref([])
const subData = ref([])

function getData(region: any, division: any) {
  let arr = []
...
function regChange(event: any) {
  divData.value = getData(event.value, null)
  divDisabled.value = false
  subDisabled.value = true
}

function divChange(event: any) {
  const val = event.value
  if (val) {
    subData.value = getData(null, event.value)
