...
export default {
  ...
  init() {
    ...
    function getAggregateEvents(dayEvents) {
      ...
      dayEvents.forEach(function (event) {
        if (event.resource.includes('_')) {
          var parentResource = event.resource.split('_')[0];
          var dayStart = new Date(event.start).setHours(0, 0, 0, 0);
    ...
    function updateCalendarEvents(inst) {
      var dailyEvents = inst.getEvents();
      var updatedSummaries = getAggregateEvents(dailyEvents);
      var updatedEvents = myEvents.concat(updatedSummaries);
      setTimeout(function () {
      ...
      var inst = $('#demo-aggregates')
        ...
        .eventcalendar({
          ...
          onEventUpdated: function (args, inst) {
            ...
            var myUpdatedEvents = myEvents.map(function (e) {
              if (e.id === updatedEvent.id) {
                return updatedEvent;
              }
              return e;
            });
            myEvents = myUpdatedEvents;
            updateCalendarEvents(inst);
          },
          ...
          onEventDeleted: function (args, inst) {
            var deletedEvent = args.event;
            var index = myEvents.indexOf(deletedEvent);
          ...
          renderResource: function (res) {
            ...
            return (
              '<div>' +
              res.car +
              '</div><div class="mds-aggregates-plate">Plate ' +
              res.plate +
              '</div><div class="mds-aggregates-cartype">' +
              res.type +
              '</div>'
            );
          },
          renderScheduleEventContent: function (event) {
            ...
            if (isParent) {
              return (
                '<div class="mds-aggregate-parent-event mbsc-flex">' +
                '<div class="mds-aggregates-details mds-aggregates-details-summary mbsc-flex mbsc-flex-1-1"><div class="mds-aggregates-no-highlight">Total dist. <div><span class="mds-aggregates-highlight-summary">' +
                event.original.distance +
                ' ml</span></div></div><div class="mds-aggregates-no-highlight">Total cons. <div><span class="mds-aggregates-highlight-summary">' +
                event.original.consumption +
                ' gal</span></div></div><div class="mds-aggregates-no-highlight">Est. cost <div><span class="mds-aggregates-highlight-summary">$' +
                Math.round(event.original.consumption * 3.706 || 0) +
                '</span></div></div></div></div></div>'
              );
            }
            return (
              '<div class="mds-aggregates-main-event">' +
              '<div class="mds-aggregates-title">' +
              event.title +
              '</div><div class="mds-aggregates-description">' +
              event.original.name +
              '</div><div class="mds-aggregates-details"><div class="mds-aggregates-no-highlight">Est. distance: <span class="mds-aggregates-highlight">' +
              event.original.distance +
              ' ml</span></div><div class="mds-aggregates-no-highlight">Est. consumption: <span class="mds-aggregates-highlight">' +
              event.original.consumption +
              ' gal</span></div></div><div class="mds-aggregates-no-highlight mbsc-flex mds-aggregates-sb"><div>Pick up: <span class="mds-aggregates-highlight""> ' +
              mobiscroll.formatDate('hh:mm A', new Date(event.original.start)) +
              '</span></div><div> Drop off: <span class="mds-aggregates-highlight">' +
  ...
  markup: `
<div id="demo-aggregates" class="demo-aggregates"></div>
  `,
  // eslint-disable-next-line es5/no-template-literals
  css: `
.demo-aggregates .mbsc-timeline-column,
.demo-aggregates .mbsc-timeline-header-column {
  width: 16em;
}

.demo-aggregates .mbsc-timeline-row-gutter {
  height: 6px;
}

.demo-aggregates .mbsc-timeline-resource-title{
  font-size: 14px;
  font-weight: 600;
}

.mds-aggregates-main-event {
  padding: 4px;
}
...
.mds-aggregates-highlight {
  font-weight: 600;
  line-height: 24px;
}

.mds-aggregates-highlight-summary {
  font-weight: 600;
  font-size: 15px;
  line-height: 24px;
}

.mds-aggregates-no-highlight {
  font-weight: 400;
}
...
.mds-aggregates-details-summary {
  padding: 8px;
}

.demo-aggregates .mbsc-schedule-event-inner {
  height: auto;
  width: 100%;
}

.mds-aggregate-parent-event {
  color: #000;
  height: 70px;
  align-items: center;
}
...
.mbsc-windows-dark .mds-aggregate-parent-event,
.mbsc-ios-dark .mds-aggregate-parent-event,
.mbsc-material-dark .mds-aggregate-parent-event {
  color: #fff;
}

.demo-aggregates .mbsc-ios .mbsc-schedule-event:after {
  display: none;
}
`,
};
