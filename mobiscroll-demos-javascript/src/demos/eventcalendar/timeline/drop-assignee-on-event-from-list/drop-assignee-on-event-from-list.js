...
export default {
  ...
  init() {
    ...
    var container = document.querySelector('.mds-drop-on-events');
    ...
    function renderEmployeeList() {
      ...
      for (var i = 0; i < employees.length; i++) {
        ...
        item.className = 'mds-drop-on-events-employee-item mbsc-flex';
      ...
      list._mdsPointerDown = function (e) {
        if (!e.target.closest('.mds-drop-on-events-employee-item')) {
          return;
        }
        function onMove() {
          container.classList.add('mds-drop-on-events-external-dragging');
          document.removeEventListener('pointermove', onMove);
        }
        function onUp() {
          container.classList.remove('mds-drop-on-events-external-dragging');
          document.removeEventListener('pointermove', onMove);
    ...
    function renderEvent(data) {
      ...
      return (
        '<div class="mds-drop-on-events-custom-event mbsc-flex" id="mds-zone-' +
        event.id +
    ...
    function initDropZones() {
      var zones = document.querySelectorAll('.mds-drop-on-events-custom-event');
      ...
      var chips = document.querySelectorAll('.mds-drop-on-events-attendee-chip');
      for (var c = 0; c < chips.length; c++) {
        chips[c].addEventListener('click', function (e) {
          e.stopPropagation();
          var empId = this.getAttribute('data-emp-id');
          var eventEl = this.closest('.mds-drop-on-events-custom-event');
