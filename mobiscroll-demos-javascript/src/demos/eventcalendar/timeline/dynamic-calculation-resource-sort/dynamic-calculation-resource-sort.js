...
export default {
  ...
  init() {
    ...
    var popup = mobiscroll.popup('#demo-filtering-popup', {
      ...
      width: 400,
      onClose: function () {
        document.querySelector('.mds-popup-sort-metric[value="' + sortColumn + '"]').checked = true;
        document.querySelector('.mds-popup-sort-direction[value="' + sortDirection + '"]').checked = true;
      },
    });
    ...
    document.querySelector('#demo-popup-sort-button').addEventListener('click', function () {
    ...
    document.querySelectorAll('.mds-popup-sort-metric').forEach(function (input) {
      input.addEventListener('change', function () {
        tempSortColumn = this.value;
        tempSortColumnLabel = this.getAttribute('data-label');
      });
    });

    document.querySelectorAll('.mds-popup-sort-direction').forEach(function (input) {
      input.addEventListener('change', function () {
        tempSortDirection = this.value;
      });
    });
  },
