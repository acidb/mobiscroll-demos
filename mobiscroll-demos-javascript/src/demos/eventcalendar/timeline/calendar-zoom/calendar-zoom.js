...
export default {
  ...
  init() {
    ...
    function handleZoom(zoom) {
      zoomLevel += zoom;
      document.querySelector('.mds-button-zoom-level-in').disabled = zoomLevel === 4;
      document.querySelector('.mds-button-zoom-level-out').disabled = zoomLevel === -4;
    ...
    document.querySelectorAll('.mds-button-zoom-level').forEach(function (button) {
      button.onclick = function () {
        return handleZoom(+button.dataset.zoom);
      };
    });
