...
To see this example live, check it out on our [demo page](https://demo.mobiscroll.com/javascript/timeline/prevent-double-booking-events#).
...
- On an [event](https://demo.mobiscroll.com/javascript/timeline/event-data-structure#) basis - by setting the `overlap` property to `false` the specified event cannot overlap.
- Per [resource](https://demo.mobiscroll.com/javascript/timeline/resource-data-structure#) - by setting the `eventOverlap` property to `false` the events in the specified resource cannot overlap.
- Globally on the instance - by setting `eventOverlap` option to `false` overlap is disbled globally.

If set to `false`, the [resource](https://demo.mobiscroll.com/javascript/timeline/resource-data-structure#) and the [event](https://demo.mobiscroll.com/javascript/timeline/event-data-structure#) settings have precedence over the global calendar  `eventOverlap` option.
