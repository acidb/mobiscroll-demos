...
To see this example live, check it out on our [demo page](https://demo.mobiscroll.com/javascript/timeline/event-grouping-bulk-action#).
...
:::inline-fw-group
prefix: The starting point is being able to look at the same dataset through different lenses. The demo supports switching between an assignee view, where each row represents a person responsible for the work, and a work type view, where each row represents a task category. A segmented control in the header allows adjusting the time granularity of the timeline while keeping a full year in view, enabling users to switch between quarterly, monthly, or weekly breakdowns. The view selector, grouping toggle, and segmented control are all rendered via the
suffix: keeping the controls integrated into the timeline header. The @if (pagemode == PageMode.Angular) { [resourceTemplate option](https://docs.mobiscroll.com/javascript/eventcalendar/timeline#template-resourceTemplate) } else if (pagemode == PageMode.Vue) { [resource slot](https://docs.mobiscroll.com/javascript/eventcalendar/timeline#slot-resource) } else { [renderResource function](https://docs.mobiscroll.com/javascript/eventcalendar/timeline#renderer-renderResource) } adapts the resource column accordingly, displaying employee avatars and job titles in assignee view, and work type color indicators in type view.
:::framework{only="angular"}
[headerTemplate](https://docs.mobiscroll.com/javascript/eventcalendar/timeline#template-headerTemplate)
option,
:::
:::framework{only="vue"}
[header](https://docs.mobiscroll.com/javascript/eventcalendar/timeline#slot-header)
slot,
:::
:::framework{only="react"}
[renderHeader](https://docs.mobiscroll.com/javascript/eventcalendar/timeline#renderer-renderHeader)
function,
:::
...
:::inline-fw-group
prefix: Once the right perspective is selected, individual events are not rendered one by one. Using the
suffix: related tasks are automatically grouped into consolidated blocks per client and per quarter. These blocks can be expanded to reveal the individual tasks inside, or collapsed to keep the timeline readable at scale.
:::framework{only="angular"}
[eventTemplate](https://docs.mobiscroll.com/javascript/eventcalendar/timeline#template-eventTemplate)
option,
:::
:::framework{only="vue"}
[timelineEvent](https://docs.mobiscroll.com/javascript/eventcalendar/timeline#slot-timelineEvent)
slot,
:::
:::framework{only="react"}
[renderTimelineEvent](https://docs.mobiscroll.com/javascript/eventcalendar/timeline#renderer-renderTimelineEvent)
function,
:::
:::framework{only="javascript"}
[renderTimelineEvent](https://docs.mobiscroll.com/javascript/eventcalendar/timeline#renderer-renderTimelineEvent)
