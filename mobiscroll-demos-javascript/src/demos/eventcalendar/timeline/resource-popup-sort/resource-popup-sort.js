...
export default {
  ...
  init() {
    ...
    var popup = mobiscroll.popup('#demo-filtering-popup', {
      buttons: [
        'cancel',
        {
          text: 'Apply',
      ...
      onClose: function () {
        mobiscroll.getInst(document.querySelector('.mbsc-popup-sort-metric[value="' + initialSortColumn + '"]')).checked = true;
        mobiscroll.getInst(document.querySelector('.mbsc-popup-sort-direction[value="' + initialSortDirection + '"]')).checked = true;
      },
    ...
    var calendar = mobiscroll.eventcalendar('#demo-timeline-popup-sort', {
      clickToCreate: true,
      data: myEvents,
      dragToCreate: true,
      dragToMove: true,
      ...
      renderResource: function (resource) {
        ...
        var barColorClass = barValue <= 33 ? 'mds-resource-green-bar' : barValue <= 66 ? 'mds-resource-yellow-bar' : 'mds-resource-red-bar';
    ...
    document.getElementById('demo-popup-sort-button').addEventListener('click', function () {
    ...
    document.querySelectorAll('.mbsc-popup-sort-metric').forEach(function (element) {
      element.addEventListener('change', function () {
        selectedMetric = this.value;
        selectedMetricDesc = this.getAttribute('data-label');
        sortColumn = selectedMetric;
      });
    });

    document.querySelectorAll('.mbsc-popup-sort-direction').forEach(function (element) {
      element.addEventListener('change', function () {
        sortDirection = document.querySelector('.mbsc-popup-sort-direction:checked').value;
      });
    });
  },
