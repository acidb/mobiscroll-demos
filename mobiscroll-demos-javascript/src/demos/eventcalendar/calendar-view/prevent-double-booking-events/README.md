...
To see this example live, check it out on our [demo page](https://demo.mobiscroll.com/javascript/eventcalendar/prevent-double-booking-events#).
...
- On an [event](https://demo.mobiscroll.com/javascript/eventcalendar/event-data-structure#) basis - by setting the `overlap` property to `false` the specified event cannot overlap.
- Globally on the instance - by setting `eventOverlap` option to `false` overlap is disbled globally.

If set to `false`, the [event](https://demo.mobiscroll.com/javascript/eventcalendar/event-data-structure#) settings have precedence over the global calendar  `eventOverlap` option.
