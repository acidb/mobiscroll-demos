...
To see this example live, check it out on our [demo page](https://demo.mobiscroll.com/javascript/scheduler/recurring-events#).
...
You can pass the recurrence rule in the `recurring` property of the event as an object or a string in `RRULE` format. [Learn about the event data structure and where to place the recurring rules](https://demo.mobiscroll.com/javascript/scheduler/event-data-structure#).

Use the configurator to experiment, build strings and objects that you can grab and use.

- **Interested in adding recurrence configuration to your UI?** [Take a look at this add/edit dialog &#8594;](https://demo.mobiscroll.com/javascript/eventcalendar/recurring-event-add-edit-dialog#)
...
- Define 4 events, each demonstrating a different `recurring` repeat type:
  ...
  1. **Daily** — `recurring: { repeat: 'daily', from: '2021-06-01', until: '2021-07-20' }`, `allDay: true`, with `recurringException: ['2021-07-05', new Date(2021, 6, 6)]` and `recurringExceptionRule: { repeat: 'monthly', day: 1 }`. `from`/`until` bound the occurrence range; `recurringException` removes specific dates (accepts both ISO strings and Date objects); `recurringExceptionRule` removes all occurrences falling on the 1st of any month.

  2. **Weekly** — `recurring: { repeat: 'weekly', weekDays: 'SA', interval: 2 }` — every other Saturday. `weekDays` accepts comma-separated day codes: `SU`, `MO`, `TU`, `WE`, `TH`, `FR`, `SA`.

  3. **Monthly** — `recurring: { repeat: 'monthly', day: 15, count: 12 }` — the 15th of each month, 12 occurrences total.

  4. **Yearly** — `recurring: { repeat: 'yearly', day: 1, month: 1 }` — January 1st every year.
