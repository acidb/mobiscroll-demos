...
To see this example live, check it out on our [demo page](https://demo.mobiscroll.com/angular/agenda/event-content-customization#).

## Demo description

Above the built in look & feel, the events can be customized in two ways:

- **Full event customization** - The agenda handles the listing and ordering of the events, while the full styling falls into your hands. Explore [this example](https://demo.mobiscroll.com/angular/agenda/full-event-customization#) for more details.
...
Pass the content template to the agendaEventContentTemplate option. All original event fields along with computed fields like `isMultiDay`, `lastDay` are available inside the template.
