To download and run this example locally, please follow the instructions [in the readme file of the project](https://github.com/acidb/mobiscroll-demos-angular?tab=readme-ov-file#mobiscroll-angular-demos).

To see this example live, check it out on our [demo page](https://demo.mobiscroll.com/angular/scheduler/group-by-resource-by-day#).

## Demo description

Resources show up in the header of the calendar as the primary group and below them are the `days`. This can be switched up through the `groupBy` option that expects `'resource'` or `'day'`.

Combined with the day and week-view you can help people quickly find what they are looking for. [Learn how to add an integrated segmented control to the header.](https://demo.mobiscroll.com/angular/scheduler/switching-calendar-scheduler-agenda#)
