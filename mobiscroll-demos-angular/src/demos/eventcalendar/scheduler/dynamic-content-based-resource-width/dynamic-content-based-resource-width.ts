...
export class AppComponent {
  ...
  icons: Record<string, { title: string; svg: string; safeSvg?: SafeHtml }> = {
    office: {
      title: 'In office',
  ...
  getIcons(resource: MyResource) {
    ...
    resourceIcons.push(this.icons[resource.role]);

    if (resource.inOffice) {
      resourceIcons.push(this.icons['office']);
    }
    if (resource.vacation) {
      resourceIcons.push(this.icons['vacation']);
    }
    if (!resource.available) {
      resourceIcons.push(this.icons['unavailable']);
    }
