...
@Component({
  selector: 'app-timeline-event-grouping-bulk-action',
  templateUrl: './event-grouping-bulk-action.html',
  encapsulation: ViewEncapsulation.None,
  styleUrl: './event-grouping-bulk-action.css',
...
export class AppComponent implements OnInit {
  ...
  groupedEvents: MbscCalendarEvent[] = [];
  myResources: any[] = assigneeResources;
  ...
  groupEventsByClientQuarter(events: MbscCalendarEvent[]): MbscCalendarEvent[] {
    ...
    events.forEach((event: any) => {
      const resourceId = this.groupBy === 'assignee' ? event.resource : event.type;
  ...
  prepareEventsForDisplay(events: MbscCalendarEvent[]): MbscCalendarEvent[] {
    return events.map((event: any) => ({
      id: event.id,
      title: event.title,
      start: event.start,
      end: event.end,
      resource: this.groupBy === 'assignee' ? event.resource : event.type,
      type: event.type,
  ...
  updateView(): void {
    const currentResources = this.groupBy === 'assignee' ? assigneeResources : typeResources;

    if (this.groupByClientQuarter) {
      this.groupedEvents = this.groupEventsByClientQuarter(this.rawEvents);
      this.displayEvents = this.groupedEvents;
    } else {
      this.groupedEvents = [];
      this.displayEvents = this.prepareEventsForDisplay(this.rawEvents);
    }
  ...
  onGroupByClientQuarterChange(event: any): void {
    this.groupByClientQuarter = event.target.checked;
    this.updateView();
  }
  ...
  getUniqueItems(originalEvents: any[]): { count: number; label: string } {
    const uniqueItems: Record<string, any> = {};

    originalEvents.forEach((ev: any) => {
    ...
    return {
      ...
      label: this.groupBy === 'assignee' ? 'type' : 'employee',
    };
  }

  getDetailInfo(event: any): { detailText: string; typeDotColor?: string; avatarUrl?: string } {
    let detailText = '';
    let typeDotColor: string | undefined;
