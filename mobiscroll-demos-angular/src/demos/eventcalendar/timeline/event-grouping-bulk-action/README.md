...
option, keeping the controls integrated into the timeline header. The @if (pagemode == PageMode.Angular) { [resourceTemplate](https://docs.mobiscroll.com/angular/eventcalendar/timeline#template-resourceTemplate)  option } else if (pagemode == PageMode.Vue) { [resource](https://docs.mobiscroll.com/angular/eventcalendar/timeline#slot-resource)  slot } else { [renderResource](https://docs.mobiscroll.com/angular/eventcalendar/timeline#renderer-renderResource)  function } adapts the resource column accordingly, displaying employee avatars and job titles in assignee view, and work type color indicators in type view.
