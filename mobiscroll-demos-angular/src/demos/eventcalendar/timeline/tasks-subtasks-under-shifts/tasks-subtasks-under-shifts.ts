...
@Component({
  selector: 'app-tasks-subtasks-under-shifts',
  styleUrl: './tasks-subtasks-under-shifts.css',
  ...
  calendarOptions: MbscEventcalendarOptions = {
    ...
    onEventCreated: (args: MbscEventCreatedEvent) => {
      ...
      if (event.shift) {
        ...
        const shiftIndex = this.myEvents.findIndex((x) => x.id === event.shift);
        ...
        if (overlapEvents.length > 2) {
          ...
          // Update the shift
          shift.tasks!.push(event.id as string);
    ...
    onEventDeleted: (args: MbscEventDeletedEvent) => {
      let newEventList = [...this.myEvents];
      const event: MyEvent = args.event;
      const evIndex = this.myEvents.findIndex((x) => x.id === event.id);
    ...
    onEventDragEnd: () => {
      this.myInvalids = blockedOutTimes;
    },
    onEventUpdateFailed: function (args: MbscEventUpdateFailedEvent) {
      const invalid: MyBlockedTimes = args.invalid;
      let msg = '';
