...
export class AppComponent implements OnInit, OnDestroy {
      ...
      if (event.start < this.now && event.end > this.now) {
        if (!event.actual) {
          ...
          const newEvent = {
            ...
            start: actualStart, end: this.now, title: 'In progress', status: 'actual',
            color: event.color, cssClass: 'mds-dispatch-actual-event mds-dispatch-pulse', editable: false,
          };
          event.actual = true;
  ...
  resetFilters(): void {
    this.searchQuery = '';
    this.myResources.forEach((cat: any) => { this.filters[cat.id] = true; });
    this.filters['operational'] = true;
    this.filters['maintenance'] = true;
    this.filterResources();
    this.refreshJobList();
  ...
  onEventClick(args: any): void {
    if (args.event.status === 'actual') {
      ...
      if (ev.title === 'In progress') {
        msg = 'Actual transport: departed ' + this.formatDelay(departureDelay) + ' · running ' + this.formatDuration(new Date().getTime() - actualStart.getTime());
      } else {
