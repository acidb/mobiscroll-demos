...
To see this example live, check it out on our [demo page](https://demo.mobiscroll.com/angular/eventcalendar/day-cell-aggregate-information#).

## Demo description

This demo showcases an advanced event calendar implementation with extensive day cell customization using the `calendarDayTemplate` , which completely overrides the default cell rendering.
...
- Enable `clickToCreate`, `dragToCreate`, `dragToMove`, `dragToResize`, and `eventDelete` for full event management in the scheduler views. Use `extendDefaultEvent` to tag drag-created events with `type: 'appointment'` and the correct color so they are counted correctly in the cell renderer.
